...
function initializeIndex(content_md){var $contentBody=$(".content-body");$(".menu_bar ul li").each(function(index){0==index?$(this).addClass("active"):$(this).removeClass("active")}),md=content_md,console.log(md),$contentBody.html(marked(content_md)),loadSlider()}function loadSlider(){var $container=$(".scroll-area"),$content=$(".article-list"),$slideArea=$(".scroll-vertical"),$bar=$(".scroll-bar");$container.slider({container:$container,content:$content,slideArea:$slideArea,bar:$bar})}var IndexController=function($scope,$interval,$http,$timeout){$scope.fetchArticlesList=function(){$http.get("/topTenArticle.json").success(function(data){$scope.detail=data.detail,$scope.articleList=data.pagination.items,$scope.f_article=data.firstArticle,initializeIndex($scope.f_article.content),initializeLoad($scope,$timeout)}).error(function(err){console.log(err)})},$scope.fetchArticlesList()};