function initializeLoad($scope,$timeout){$scope.start(),$scope.fakeIntro=!0,$timeout(function(){$scope.complete(),$scope.fakeIntro=!1,initializeHighLight()},750)}function initializeHighLight(){console.log("加载代码高亮"),$("pre code").each(function(i,block){hljs.highlightBlock(block)})}var pBlogApp={},app=angular.module("pBlogApp",["chieffancypants.loadingBar","ngAnimate","ngRoute","ngSanitize","pBlogApp.filters","pBlogApp.services","pBlogApp.directives"]).config(function($routeProvider,$locationProvider){$routeProvider.when("/index",{templateUrl:"/index/layout",controller:IndexController}),$routeProvider.when("/archives",{templateUrl:"/archives/layout",controller:ArchivesController}),$routeProvider.when("/category",{templateUrl:"/category/layout",controller:CategoryController}),$routeProvider.when("/archives/:timeStamp",{templateUrl:"/archives/layout",controller:ArchivesTimeStampController}),$routeProvider.when("/category/:categorySlug",{templateUrl:"/category/layout",controller:CategorySlugController}),$routeProvider.when("/article/:articleSlug",{templateUrl:"/article/slug/layout",controller:ArticleController}),$routeProvider.when("/about",{templateUrl:"/about/layout",controller:AboutController}),$routeProvider.otherwise({redirectTo:"/index"})}).config(function(cfpLoadingBarProvider){cfpLoadingBarProvider.includeSpinner=!0}).controller("loadingBar",function($scope,$http,$timeout,cfpLoadingBar){$scope.start=function(){cfpLoadingBar.start()},$scope.complete=function(){cfpLoadingBar.complete()}});
...
function initializeAbout(){$(".menu_bar ul li").each(function(index){5==index?$(this).addClass("active"):$(this).removeClass("active")})}var AboutController=function($scope,$interval,$http,$timeout){$scope.fetchAboutMessage=function(){$http.get("/about.json").success(function(data){initializeAbout(),initializeLoad($scope,$timeout)}).error(function(err){console.log(err)})},$scope.fetchAboutMessage()};
